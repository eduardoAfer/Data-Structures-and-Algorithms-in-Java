...
public class BTree<T> {
   ...
   // retorna a quantidade de nodos internos
   public int internal() {
      return internal(root);
   }

   public int internal(BTNode<T> nodo) {
      if (nodo == null)
         return 0;
      if (nodo.getLeft() != null || nodo.getRight() != null)
         return 1 + internal(nodo.getLeft()) + internal(nodo.getRight());

      return internal(nodo.getLeft()) + internal(nodo.getRight());
   }
   ...
   private void cut(BTNode<T> n, int d, int contador) {
      if (d <= 0)
         root = null;
      if (n == null || contador > d)
         return;
      if (contador == d) {
         n.setLeft(null);
         n.setRight(null);
      ...
      cut(n.getLeft(), d, contador + 1);
      cut(n.getRight(), d, contador + 1);

   }

   // ----------------------------------------------------
   public int qntMaxOfLevel = 0;

   public int[] maxLevel() {

      int[] nodosNoNivel = new int[depth() + 1];// armazena qnt nodos por nivel e seu indice é o nivel
      int[] aux = new int[2];
      aux[0] = maxLevel(root, 0, nodosNoNivel, aux);
      aux[1] = qntMaxOfLevel;
   ...
   public int maxLevel(BTNode<T> nodo, int nivel, int[] nodosNoNivel, int[] aux) {

      if (nodo != null) {
         nodosNoNivel[nivel] += 1;
         if (nodosNoNivel[nivel] > aux[0]) {
            aux[0] = nodosNoNivel[nivel];
            qntMaxOfLevel = 0; // variável global, contem as vezes que qnt maxima aparece
         }
         if (nodosNoNivel[nivel] == aux[0]) {
            qntMaxOfLevel++;
         }

         maxLevel(nodo.getLeft(), nivel + 1, nodosNoNivel, aux);
         maxLevel(nodo.getRight(), nivel + 1, nodosNoNivel, aux);
